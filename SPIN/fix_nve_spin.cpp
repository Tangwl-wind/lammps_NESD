  ...
  backward_stacks(nullptr), forward_stacks(nullptr)
{
  ...
  while (iarg < narg) {
    ...
    // read cutoff consistent to dp model
    if (strcmp(arg[iarg],"cutoff") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix/nve/spin command, error in cutoff parameter");
      cutoff_dp = utils::numeric(FLERR,arg[iarg+1],false,lmp);
      iarg += 2;
    } else error->all(FLERR,"Illegal fix/nve/spin command, lack of cutoff from dp model");
  }
...
void FixNVESpin::initial_integrate(int extend_vflag)
{
  ...
  // divide extend_vflag into true eflag and vflag
...
void FixNVESpin::sectoring()
{
  ...
  // read cutoff from dp model

  if (cutoff_dp == 0.0)
   error->all(FLERR,"Illegal sectoring operation, dp model cutoff is 0");

  double rax = rsx / cutoff_dp;
  double ray = rsy / cutoff_dp;
  double raz = rsz / cutoff_dp;
  ...
  if (sector_flag == 1 && nsectors != 8)
    error->all(FLERR,"Illegal sectoring operation, cutoff is too large for sectoring");
